...
//      val labelset = "("+Columns.toArray().map(v=>"'"+v.attr+"'").mkString(",")+")"
      val srcidSet = ids.map(v=>arrow(v._1,v._2)).repartitionByCassandraReplica("mykeyspace",tableName)
...
//      println("join size : "+srcidSet.collect().foreach(println))
    ...
    def main(args:Array[String]):Unit = {
      val sparkConf = new SparkConf().setAppName("HBaseMultipleThread").setMaster("local[3]")
      .set("spark.cassandra.connection.host", "127.0.0.1")
      val sc = new SparkContext(sparkConf)
      val srcidSet = sc.parallelize(1L to 26L, 3).cartesian(sc.parallelize(Array("video"), 3)).map(v=>arrow(v._1,v._2)).repartitionByCassandraReplica("mykeyspace","testgraph")
                        .joinWithCassandraTable("mykeyspace","testgraph")
      println(srcidSet.collect().length)
    }
}